...
namespace TaskA.Controllers
{
    ...
    public class OrdersController : Controller
    {
        ...
        public ActionResult AddNew(int id)
        {
            ...
            $"Here is your reference number: " + getReference + ". you can track and view the progress of the tasker while you are away: https://21grp35.azurewebsites.net/Orders/TrackOrder/ " +
            $"<br/>" + "You can download your invoice here: " + $" https://21grp35.azurewebsites.net/QRCodes/Details/{getInvoice}" +" and provide it to the driver wheh he/she arrives to drop the tasker"+
            $"<br/>" +
            $"Task A";
        ...
        public ActionResult Create([Bind(Include = "Order_ID,Order_DateTime,Book_ID,Tasker_ID,Driver_ID")] Order order)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Order_ID,Order_DateTime,Book_ID,Tasker_ID,Driver_ID")] Order order)
        {
            using (var context = new ApplicationDbContext())
            {
                ViewBag.Count = context.Orders.SqlQuery(" SELECT * FROM dbo.Orders").Count();
            }
                ...
                order.OrderStatus_ID = order.GetStatus("Arrived for service rendering");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateStatus([Bind(Include = "Order_ID,Order_DateTime,Book_ID,Tasker_ID,Driver_ID")] Order order)
        {
            using (var context = new ApplicationDbContext())
            {
                ViewBag.Count = context.Orders.SqlQuery(" SELECT * FROM dbo.Orders").Count();
        ...
        //{
        //    ViewBag.Book_ID = new SelectList(db.BookingStatus, "Book_ID", "Book_RecipientName", order.Book_ID);
